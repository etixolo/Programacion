...
include 'autentificacion.php'; // Funciones de registro y login.
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $nombre_usuario = trim($_POST['nombre_usuario']); // Nombre de usuario.
    $correo = trim($_POST['correo']); // Correo electrónico.
    $password = trim($_POST['password']); // Contraseña.

    // Intenta registrar al usuario.
    $resultado = registrarUsuario($nombre_usuario, $correo, $password, $conexion);

    // Si el registro fue exitoso.
    if ($resultado === true) {
        header('Location: iniciosesion.php'); // Redirige a la página de login.
        exit; // Termina el script.
    } else {
    ...
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-6">
                ...
                <?php endif; ?>
                <form action="registro.php" method="POST" class="bg-white p-4 rounded shadow">
                    <div class="mb-3">
                ...
                <p class="text-center mt-3">¿Ya tienes una cuenta? <a href="iniciosesion.php">Inicia sesión aquí</a>.</p>
            </div>
        </div>
    </div>
</body>
